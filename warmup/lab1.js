 ...
 * Task 1
 */
const simpleArraySum = xs => xs.reduce((acc, cur) => acc + cur, 0); 
...
const generateRandomItemList = n => {
  ...
  for (let i = 0; i < n; i++) {
    const item = {
      name: "item" + getRandomIntInclusive(0,10000),
      cost: getRandomIntInclusive(0,10000)
    }
    items.push(item);
  }
  return items;
 ...
 * Task 4
 */
const findMostExpensiveItem = items => items.sort((a,b) => b.cost - a.cost)[0];
 ...
 * Task 5
 */
const findCheapestItem = items => items.sort((a,b) => a.cost - b.cost)[0];
 ...
 * 
 */
const searchItems = (items, re) => items.filter(item => item.name.match(re) !== null);
